...
int i2c_write(unsigned char slave_addr, unsigned char reg_addr,
    ...
    #else 
    num_bytes_read = i2c_write_blocking(i2c_default, slave_addr, msg, (length + 1), false);
    #endif
...
int i2c_read(unsigned char slave_addr, unsigned char reg_addr,
    ...
    #ifdef USE_I2C1
    i2c_write_blocking(i2c1, slave_addr, &reg, 1, true);
    if(num_bytes_read == -1) return -1;
...
static inline int pico_reg_int_cb(struct int_param_s *int_param) {
    if(int_param->cb == NULL) {
        gpio_set_pulls(int_param->int_pin, false, false);
        gpio_deinit(int_param->int_pin);
        return 0;
    }
    gpio_init(int_param->int_pin);
    gpio_set_dir(int_param->int_pin, false);
    gpio_pull_up(int_param->int_pin);
    gpio_set_irq_enabled_with_callback(int_param->int_pin, int_param->int_event, true, int_param->cb);
    return 0;
}
#define reg_int_cb pico_reg_int_cb
#define labs labs
#define min(a,b) ((a<b)?a:b)
#define fabsf fabsf
#define fabs fabsf
 ...
 * 
 */
int mpu_get_biases(long * gyro, long * accel) {
    int hw_test = 0;
...
int mpu_find_gyro_calibration_biases(long * gyro) {
    ...
    short current_gyro_fsr = 0;
    mpu_get_gyro_fsr(&current_gyro_fsr);
    ...
    for(int i = 0; i < 1000; i++) {
        ...
        gyro_sum[0] += gyro_values[0];
        gyro_sum[1] += gyro_values[1];
        gyro_sum[2] += gyro_values[2];
        sleep_ms(1);
    }
    for(int i = 0; i < 3; i++){
...
int mpu_find_accel_calibration_biases_pid(long * accel) {
    ...
    int16_t accel_p[] = {0, 0, 0};
    int16_t accel_i[] = {0, 0, 0};

    char current_accel_fsr = 0;
    mpu_get_accel_fsr(&current_accel_fsr);
    ...
    for(int i = 0; i < 2000; i++) {
        ...
        for(int j = 0; j < 3; j++) {
            /* Calculate Error */
            int16_t accel_error = (accel_values[j] - accel_expected[j]);
            /* Calculate Proportional */
            accel_p[j] = accel_error * accel_kp;
            /* Calculate integral */
            accel_i[j] = accel_error * accel_ki; 
            if(accel_i[j] > 500) 
                accel_i[j] = 500;
            else if(accel_i[j] < -500) 
                accel_i[j] = -500;
            /* Enter new bias values */
            accel[j] = (accel_p[j]+accel_i[j]) * 9.80665f / 16.0f;
        }
 ...
 * The following HPF settings are supported:    MPU6050_DHPF_RESET = 0,
                                                MPU6050_DHPF_5HZ = 1,
                                                MPU6050_DHPF_2_5HZ = 2,
                                                MPU6050_DHPF_1_25HZ = 3,
                                                MPU6050_DHPF_0_63HZ = 4,
                                                MPU6050_DHPF_HOLD = 7.
    @param[in]  dhpf Desired HPF setting.
    @return     0 if succesful. 
 */
int mpu_set_hpf(enum MPU6050_DHPF dhpf) {
    unsigned char value;
    if(mpu_read_reg(reg.accel_cfg, &value)){
        return -1;
    }
    value &= 0xF8;
    value |= dhpf;
    if(i2c_write(st.hw->addr, reg.accel_cfg, 1, &value)) {
        return -1;
    }
    st.chip_cfg.hpf = dhpf;
    return 0;
}
...
restore:
     ...
     */
    get_st_biases(gyro, accel, 0);st.chip_cfg.sensors
    result = 0x7;
#endif
    /* Set to invalid values to ensure no I2C writes are skipped. */
    st.chip_cfg.gyro_fsr = 0xFF;
